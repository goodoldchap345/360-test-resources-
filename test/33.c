...
struct Link *map_list(struct Link *list, int (*trans_func)(int)) {
    struct Link* newList = NULL;
    struct Link* currentPointer = list;

    while (currentPointer != NULL) {
        int val = currentPointer->value;
        val = trans_func(val);

        newList = append(val, newList);
